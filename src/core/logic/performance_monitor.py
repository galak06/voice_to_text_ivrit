...
class PerformanceMonitor:
    ...
    def __init__(self, monitoring_interval: float = 5.0, max_history_size: int = 1000):
        """
        Initialize performance monitor with resource limits
        ...
        if not dependency_manager.is_available('psutil'):
            logger.warning("⚠️ psutil not available - performance monitoring will be limited")
            self.process = None
            self._baseline_io = None
            self._baseline_network = None
        else:
            psutil = dependency_manager.get_module('psutil')
    ...
    def measure_performance(self) -> PerformanceMetrics:
        ...
        try:
            ...
            # Check if psutil is available
            if not dependency_manager.is_available('psutil') or self.process is None:
